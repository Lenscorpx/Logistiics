...
{
    public partial class frm_structures : MetroForm
    {
        public frm_structures()
        {
            InitializeComponent();
            refreshDatas();
        }
        ...
        private void afficher_structure()
        {
        ...
        private void recuperer_zone()
        {
            ...
            try
            {
                if (cnx.State == ConnectionState.Closed)
                    cnx.Open();
                var cmd = new SqlCommand("recuperer_zone", cnx)
                {
                    CommandType = CommandType.StoredProcedure
                };
                cmd.ExecuteNonQuery();
        ...
        {
            txt_id_structure.Text = dtgrid.SelectedRows[0].Cells[0].Value.ToString();
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (cnx.State == ConnectionState.Closed)
                    cnx.Open();
                const string requete = "enregistrer_structure";
                var cmd = new SqlCommand(requete, cnx)
                {
                    CommandType = CommandType.StoredProcedure
                };
                cmd.Parameters.Add(new SqlParameter("id_structure", SqlDbType.NVarChar)).Value = txt_id_structure.Text;
                //cmd.Parameters.Add(new SqlParameter("date_approvisionnement", SqlDbType.Date)).Value = Convert.ToDateTime(dtp_date_approv.Text);
                cmd.Parameters.Add(new SqlParameter("@descr_structure", SqlDbType.NVarChar)).Value = richTextBox1.Text;
                ...
                cmd.Parameters.Add(new SqlParameter("id_zone", SqlDbType.NVarChar)).Value = cbx_ville.Text;
